<#
.SYNOPSIS
    Build, sign, and package Phosphor into a Windows installer.

.DESCRIPTION
    Mirrors the macOS build_bundle.sh / build_pkg.sh model (env-var driven,
    with an ad-hoc / self-signed fallback) for Windows:

      1. cargo build --release         (static-CRT via .cargo/config.toml)
      2. sign target\release\phosphor.exe
      3. iscc windows\phosphor.iss     -> dist\Phosphor-<ver>-windows-x86_64-setup.exe
...
.EXAMPLE
    $env:WIN_CERT_PFX = 'C:\certs\phosphor.pfx'
...
$ExePath  = Join-Path $RepoRoot 'target\release\phosphor.exe'
$IssPath  = Join-Path $RepoRoot 'windows\phosphor.iss'
...
Write-Step "Phosphor version $Version"
...
function Find-SignTool {
    ...
    # Prefer newest SDK version if the glob matched several.
...
# --- Signing ----------------------------------------------------------------
# Resolve the signing strategy once, up front, so we fail fast on bad config.
$SignTool = $null
...
elseif ($SelfSign) {
    ...
    $subject = 'CN=Phosphor Dev (self-signed)'
...
function Invoke-Sign {
    ...
    # self-signed cert never does, so verifying it would always fail — the
    # signature is still validly attached, which is all the -SelfSign test path
    # needs. Only enforce the trust check for real certificates.
    if ($IsSelfSigned) {
...
$Installer = Join-Path $DistDir "Phosphor-$Version-windows-x86_64-setup.exe"
