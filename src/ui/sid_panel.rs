...
const DECAY_TIMES:  [&str; 16] = [
    ...
    "300ms","750ms","1.5s","2.4s","3s","9s","15s","24s",
];
...
const SID_ACCENT: [Color; 4] = [
    Color { r: 0.30, g: 0.85, b: 0.55, a: 1.0 }, // SID1 – green
    Color { r: 0.40, g: 0.60, b: 0.95, a: 1.0 }, // SID2 – blue
    Color { r: 0.90, g: 0.55, b: 0.30, a: 1.0 }, // SID3 – orange
    Color { r: 0.85, g: 0.35, b: 0.55, a: 1.0 }, // SID4 – pink
];

fn dim(c: Color, factor: f32) -> Color {
    Color { r: c.r * factor, g: c.g * factor, b: c.b * factor, a: c.a }
}
...
pub fn sid_panel<'a>(
    ...
    let content = iced::widget::Row::with_children(
        chips.into_iter().enumerate().flat_map(|(i, chip)| {
            if i == 0 {
                vec![chip]
            } else {
...
fn sid_chip_panel<'a>(regs: &[u8], sid: usize, is_pal: bool) -> Element<'a, Message> {
    ...
    let label  = if sid == 0 { "SID 1".to_string() } else { format!("SID {}", sid + 1) };
...
fn voice_row<'a>(
    ...
    let hz_str  = if hz < 1.0 { "0 Hz".to_string() } else { format!("{:.0} Hz", hz) };
    ...
    if sync { flags.push_str(" SYN"); }
    if ring { flags.push_str(" RNG"); }
    if test { flags.push_str(" TST"); }
    ...
    let pulse_row: Element<'a, Message> = row![
        Space::new().width(26),
        label_text("PW", if pulse_active { dim_color } else { Color::TRANSPARENT }),
        Space::new().width(4),
        label_text(
            if pulse_active { format!("{:.0}%  (${:03X})", pw_pct, pw_val) } else { String::new() },
            val_color,
        ),
    ]
    ...
    column![
        ...
        row![
            text(voice_label).size(12).color(label_color).width(Length::Fixed(20.0)),
            Space::new().width(6),
            text(wave).size(12).color(val_color).width(Length::Fixed(68.0)),
        ...
        row![
            Space::new().width(26),
            text(note).size(12).color(val_color).width(Length::Fixed(40.0)),
...
fn global_row<'a>(regs: &[u8], base: usize, accent: Color) -> Element<'a, Message> {
    ...
    if lp { mode.push_str("LP "); }
    if bp { mode.push_str("BP "); }
    if hp { mode.push_str("HP "); }
    if mode.is_empty() { mode.push_str("---"); }
    let mode = mode.trim_end().to_string();

    // Build routing string
    let mut routing = String::new();
    if route_v1  { routing.push_str("V1 "); }
    if route_v2  { routing.push_str("V2 "); }
    if route_v3  { routing.push_str("V3 "); }
    if route_ext { routing.push_str("EXT"); }
    if routing.is_empty() { routing.push_str("none"); }
    ...
    let v3off_badge: Element<'a, Message> = row![
        ...
        label_text(
            ...
            if v3_off { Color::from_rgb(0.9, 0.5, 0.3) } else { Color::TRANSPARENT },
        ),
    ].into();
